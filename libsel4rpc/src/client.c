                           ...
                           seL4_CPtr *caps_to_send, uint32_t n_caps_to_send)
{
    ...
    if (root != seL4_CapNull)
    {
        seL4_SetCapReceivePath(root, capPtr, capDepth);
    }

    /* set caps to send */
    for (int i = 0; i < n_caps_to_send; i++)
    {
        seL4_SetCap(i, caps_to_send[i]);
    }
    
    /* set magic header */
    seL4_SetMR(0, client->magic);
    seL4_Call(client->server_ep, seL4_MessageInfo_new(0, 0, n_caps_to_send, stream_size));
...
int sel4rpc_call(sel4rpc_client_t *client, RpcMessage *msg,
                 seL4_CPtr root, seL4_CPtr capPtr, seL4_Word capDepth)
{
    return sel4rpc_call_with_caps(client, msg, root, capPtr, capDepth, NULL, 0);
}